...
def create_invoice(chat_id):
    ...
    payment = Payment.create({
        ...
        "confirmation": {
            "type": "redirect",
            "return_url": "https://google.com"  # какой-то свой адрес
        },
        "capture": True,
        "description": "Заказ №1",
        "metadata": {"chat_id": chat_id}  # юкасса после оплаты пошлет объект с помощью  вэбхука
    })

    return payment.confirmation.return_url


def get_from_env(key):
    dotenv_path = join(dirname(__file__), ".env")
    load_dotenv(dotenv_path)
    return os.environ.get(key)  # возвращает токен из .env
...
def process():
    if check_if_successful_payment(request):
        ...
        send_message(chat_id, "oplata proshla uspeshno")
    else:
        #обработка от телеграм
        chat_id = request.json["message"]["chat"]["id"]  # обращаюсь в многомерный массив по ключам
        send_pay_button(chat_id=chat_id, text="test pay")
